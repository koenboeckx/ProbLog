 % Rational Rules $\label{lst:rational_rules}$
...
:-use_module('generate_unique_id.py').  % Help with noisy_equal: 
...
conj([Symbol|Conj], N, RuleID) :-
    N > 0,
    select_uniform(conj_id(N, RuleID), [0,1,?], Symbol, _), % uncomment to enable  don't care
    %select_uniform(conj_id(N, RuleID), [0,1], Symbol, _),  % uncomment to disable don't care
    N1 is N-1,
    conj(Conj, N1, RuleID).
...
observe :-
	max_len_rule(Max),
	formula(F, Max),
	eval_pos(F, [1,1]). % one oberved positive example
